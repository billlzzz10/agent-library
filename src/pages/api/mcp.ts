...
function createServer(options: ServerOptions = {}) {
  ...
  server.server.setRequestHandler(GetPromptRequestSchema, async (request) => {
    ...
    // Performance Optimization: Direct indexed lookup for slug or id first
    // instead of fetching all matching prompts into memory.
    ...
    // Fallback for unslugified legacy titles: fetch prompts and match via slugify(title)
    if (!prompt) {
      const prompts = await db.prompt.findMany({
        where: promptFilter,
        select: {
