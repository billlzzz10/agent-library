...
/**
 * Extract features from content to be used for multiple similarity checks
 * This avoids redundant normalization and tokenization in bulk comparisons
 */
export function extractFeatures(content: string): SimilarityFeatures {
  ...
  // N-gram set (trigrams) for sequence similarity
 ...
 * Optimized to O(min(N, M)) by avoiding intermediate arrays/sets
 */
...
/**
 * Check if two pre-extracted features are similar enough to be considered duplicates
 * This is the high-performance version for bulk comparisons
 */
export function isSimilarContentWithFeatures(
  ...
  return (jaccard * 0.6 + ngram * 0.4) >= threshold;
}
