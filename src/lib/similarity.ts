...
/**
 * Helper to get n-grams (trigrams by default)
 */
...
/**
 * Efficiently calculate similarity between two sets (Jaccard)
 * Avoids creating intermediate arrays or sets to reduce GC pressure
 */
...
/**
 * Extract features from content once to be reused in multiple comparisons
 */
export function extractFeatures(content: string): SimilarityFeatures {
  ...
  const trigramSet = getNgrams(normalized, 3);
...
/**
 * Check if two contents are similar enough using pre-extracted features
 */
