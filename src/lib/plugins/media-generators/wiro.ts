...
export const wiroGeneratorPlugin: MediaGeneratorPlugin = {
  ...
  async startGeneration(request: GenerationRequest): Promise<GenerationTask> {
    ...
    if (request.inputImageUrl) {
      // A10: Validate input image URL is not targeting private/internal networks
      if (isPrivateUrl(request.inputImageUrl)) {
        throw new Error("Input image URL targets a private/internal network which is not allowed");
      }
