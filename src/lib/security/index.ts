...
export function isPrivateUrl(urlString: string | undefined | null): boolean {
  ...
  try {
    ...
    if (hostname === 'localhost' || hostname === '127.0.0.1' || hostname === '::1' || hostname === '0.0.0.0' || hostname === '::' || hostname === '[::]') {
      return true;
    }
    ...
    // Node's URL hostname for IPv6 includes square brackets
    if (hostname.startsWith('[')) {
      const ipv6 = hostname.slice(1, -1).toLowerCase();
      // Simple checks for common IPv6 internal/reserved prefixes
      if (
        ipv6 === '::1' ||
        ipv6 === '::' ||
    ...
    // Invalid URL - treat as potentially dangerous if we can't parse it
    return true;
  }
}
