...
export function isPrivateUrl(urlString: string): boolean {
  try {
    ...
    // Node's URL implementation normalizes most numeric formats to dotted-decimal for IPv4
    ...
    // Node normalizes many IPv6 formats, including IPv4-mapped ones
    if (hostname.startsWith('[') && hostname.endsWith(']')) {
      ...
      if (
        ...
        ipv6.startsWith('fc') ||
        ipv6.startsWith('fd') ||
        ipv6.startsWith('::ffff:7f') // IPv4-mapped loopback (normalized from [::ffff:127.0.0.1])
      ) {
        return true;
      }
    }
