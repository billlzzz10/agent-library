...
describe("isPrivateUrl", () => {
  it("should return true for private IPv4 addresses and loopbacks", async () => {
  ...
  it("should return true for invalid or non-http protocols", async () => {
    expect(await isPrivateUrl("file:///etc/passwd")).toBe(true);
    expect(await isPrivateUrl("gopher://127.0.0.1")).toBe(true);
    expect(await isPrivateUrl("not-a-url")).toBe(true);
  });

  it("should return false for valid public URLs", async () => {
    expect(await isPrivateUrl("https://hooks.slack.com/services/test/test")).toBe(false);
