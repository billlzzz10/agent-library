...
describe("Rebranding Sweep", () => {
  it("should fail if old URL references are found", () => {
    let output = "";
    try {
      output = execSync('git grep -l "awesome-chatgpt-prompts" -- ":(exclude)src/__tests__" ":(exclude)node_modules" ":(exclude).git" ":(exclude)package-lock.json" ":(exclude)conductor/tracks" ":(exclude).gemini"', { encoding: 'utf8' });
    } catch (_error) {
