...
import { extractFeatures, calculateSimilarityWithFeatures, DEFAULT_SIMILARITY_THRESHOLD } from "@/lib/similarity";
...
export async function POST(request: Request) {
  try {
    ...
    if (newContentFeatures.normalized.length > 50) {
      ...
      // Optimization: Extract features for each public prompt once and use pre-extracted features for comparison
      const similarPrompt = publicPrompts.find(p => {
        const existingFeatures = extractFeatures(p.content);
        return calculateSimilarityWithFeatures(newContentFeatures, existingFeatures) >= DEFAULT_SIMILARITY_THRESHOLD;
      });
