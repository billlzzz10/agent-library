...
export async function POST(request: Request) {
  try {
    ...
    // Pre-extract features for the new content to avoid redundant processing in the loop
    ...
    if (newContentFeatures.normalized.length > 50) {
      ...
      // Optimization: extractFeatures for p.content is still called in the loop,
      // but newContentFeatures is extracted only once.
      const similarPrompt = publicPrompts.find(p =>
        isSimilarContentWithFeatures(newContentFeatures, extractFeatures(p.content))
      );
