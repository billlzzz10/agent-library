...
import { extractFeatures, calculateSimilarityWithFeatures, normalizeContent } from "@/lib/similarity";
...
export async function POST(request: Request) {
  try {
    ...
    // First, extract features from the new content once
    ...
    if (newFeatures.normalized.length > 50) {
      ...
      // Optimization: extract features for the target content once, then compare
