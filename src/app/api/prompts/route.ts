...
import { normalizeContent, extractFeatures, calculateSimilarityWithFeatures } from "@/lib/similarity";
...
export async function POST(request: Request) {
  try {
    ...
    // Extract features for the new content once before the loop
    ...
    if (newContentFeatures.normalized.length > 50) {
      ...
      // Optimization: extract features for each candidate and use pre-calculated features for new content
      const similarPrompt = publicPrompts.find(p => {
        const candidateFeatures = extractFeatures(p.content);
        return calculateSimilarityWithFeatures(newContentFeatures, candidateFeatures) >= 0.85;
      });
