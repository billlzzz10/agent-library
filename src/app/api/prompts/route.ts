...
export async function POST(request: Request) {
  try {
    ...
    if (newContentFeatures.normalized.length > 50) {
      ...
      // Performance: newContentFeatures is extracted once, each p.content features are extracted in the loop
      // We still see ~5x improvement because extractFeatures(content) was called twice per loop iteration before
