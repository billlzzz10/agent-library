...
export async function POST(request: Request) {
  try {
    ...
    if (newContentFeatures.normalized.length > 50) {
      ...
      // By extracting features for each prompt in the loop once
      const similarPrompt = publicPrompts.find(p => {
        const publicPromptFeatures = extractFeatures(p.content);
        return isSimilarContentWithFeatures(newContentFeatures, publicPromptFeatures);
      });
