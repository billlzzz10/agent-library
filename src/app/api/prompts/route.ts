...
export async function POST(request: Request) {
  try {
    ...
    // First, pre-calculate features for the new content
    ...
    if (newFeatures.normalized.length > 50) {
      ...
      // Find similar content using optimized similarity check
      const similarPrompt = publicPrompts.find(p => {
        const existingFeatures = extractFeatures(p.content);
        return calculateSimilarityWithFeatures(newFeatures, existingFeatures) >= DEFAULT_SIMILARITY_THRESHOLD;
      });
