...
export async function POST(request: Request) {
  try {
    ...
    // First, extract features for the new prompt for efficient batch comparison
    ...
    if (newFeatures.normalized.length > 50) {
      ...
      // We extract features for each existing prompt in the batch
      // The new prompt's features are extracted only once above
      const similarPrompt = publicPrompts.find(p => {
        const existingFeatures = extractFeatures(p.content);
        return calculateSimilarityWithFeatures(newFeatures, existingFeatures) >= 0.85;
      });
