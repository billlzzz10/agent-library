...
import { isSimilarContentNormalized, getSimilarityFeatures, normalizeContent } from "@/lib/similarity";
...
export async function POST(request: Request) {
  try {
    ...
    // Pre-calculate features for the new content once before the loop
    const newContentFeatures = getSimilarityFeatures(content);
    ...
    if (newContentFeatures.normalized.length > 50) {
      ...
      // We normalize the comparison content in the loop, but keep the new content features pre-calculated
      const similarPrompt = publicPrompts.find(p => {
        const pFeatures = getSimilarityFeatures(p.content);
        return isSimilarContentNormalized(newContentFeatures, pFeatures);
      });
