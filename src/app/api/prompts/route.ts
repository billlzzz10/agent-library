...
export async function POST(request: Request) {
  try {
    ...
    if (newFeatures.normalized.length > 50) {
      ...
      // Optimization: extract features for each existing prompt and use the optimized comparison
      const similarPrompt = publicPrompts.find(p => {
        const existingFeatures = extractFeatures(p.content);
        const similarity = calculateSimilarityWithFeatures(newFeatures, existingFeatures);
        return similarity >= DEFAULT_SIMILARITY_THRESHOLD;
      });
