...
import { isSimilarContent, normalizeContent, extractFeatures, calculateSimilarityWithFeatures } from "@/lib/similarity";
...
export async function POST(request: Request) {
  try {
    ...
    // Extract features once for the new content to avoid redundant processing in the loop
    ...
    if (newContentFeatures.normalized.length > 50) {
      ...
      // Find similar content using optimized comparison with pre-extracted features
      const similarPrompt = publicPrompts.find(p => {
        const existingFeatures = extractFeatures(p.content);
        return calculateSimilarityWithFeatures(newContentFeatures, existingFeatures) >= 0.85;
      });
