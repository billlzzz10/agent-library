...
import { isSimilarContentWithFeatures, extractFeatures } from "@/lib/similarity";
...
export async function POST(request: Request) {
  try {
    ...
    if (newContentFeatures.normalized.length > 50) {
      ...
      // Find similar content using our similarity algorithm optimized for bulk comparison
      // By extracting features of the new content once, we avoid redundant normalization
      // and tokenization for each of the 1000 comparisons.
      const similarPrompt = publicPrompts.find(p => {
        const pFeatures = extractFeatures(p.content);
        return isSimilarContentWithFeatures(newContentFeatures, pFeatures);
      });
