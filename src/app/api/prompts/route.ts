...
export async function POST(request: Request) {
  try {
    ...
    if (newContentFeatures.normalized.length > 50) {
      ...
      // Optimization: extract features for the new content once, and compare against each public prompt
      const similarPrompt = publicPrompts.find(p => {
        const existingFeatures = extractFeatures(p.content);
        return calculateSimilarityWithFeatures(newContentFeatures, existingFeatures) >= 0.85; // 0.85 is default threshold
      });
