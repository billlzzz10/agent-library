...
export async function POST(request: Request) {
  try {
    ...
    // Pre-extract features for the newly submitted prompt content
    ...
    if (newPromptFeatures.normalized.length > 50) {
      ...
      // Pre-extract features of the candidate prompts inside the search to minimize processing overhead.
      const similarPrompt = publicPrompts.find((p) => {
        const candidateFeatures = extractFeatures(p.content);
        return calculateSimilarityWithFeatures(newPromptFeatures, candidateFeatures) >= 0.85;
      });
